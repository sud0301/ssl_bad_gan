...
class Trainer(object):
    ...
    def __init__(self, config, args):
        ...
        self.labeled_loader, self.unlabeled_loader, self.unlabeled_loader2, self.dev_loader, self.special_set = data.get_gris_loaders(config)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='gris_trainer.py')
    parser.add_argument('-suffix', default='run0', type=str, help="Suffix added to the save images.")

    args = parser.parse_args()

    trainer = Trainer(config.gris_config(), args)
    trainer.train()
